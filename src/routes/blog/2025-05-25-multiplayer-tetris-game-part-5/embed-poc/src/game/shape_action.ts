...
export type ShapeAction = (shape: import("./shape").Shape) => void;

export const SHAPE_ACTIONS: { [ShapeActionName: string]: ShapeAction } = {
  [ShapeActionName.MOVE_LEFT]: (shape) => {
    shape.posX--;
  },
  [ShapeActionName.MOVE_RIGHT]: (shape) => {
    shape.posX++;
  },
  [ShapeActionName.MOVE_DOWN]: (shape) => {
    shape.posY++;
  },
  [ShapeActionName.ROTATE]: (shape) => {
    const type: number[][] = [];
