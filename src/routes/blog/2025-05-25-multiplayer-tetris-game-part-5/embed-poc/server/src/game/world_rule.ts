...
export const createWorldRules = (): WorldRule[] => {
  const isBlockedFromLeft = (params: WorldRuleParams): WORLD_ACTION | null => {
    if (params.action !== ShapeActionName.MOVE_LEFT) return null;
    if (params.sim.posX < 0) return WORLD_ACTION.BLOCK_SHAPE_ACTION;
    for (const [shape] of getOtherShapes(params.shape, params.shapes)) {
      if (!doesBBoxOverlap(params.sim, shape)) continue;
      const col = 0;
  ...
  const isBlockedFromRight = (params: WorldRuleParams): WORLD_ACTION | null => {
    if (params.action !== ShapeActionName.MOVE_RIGHT) return null;
  ...
  const isBlockedFromDown = (params: WorldRuleParams): WORLD_ACTION | null => {
    if (params.action !== ShapeActionName.MOVE_DOWN) return null;
    if (params.sim.posY + params.sim.type.length > WORLD_H)
      return WORLD_ACTION.STOP_SHAPE_ACTIONS;
    for (const [shape, isActive] of getOtherShapes(
      params.shape,
      params.shapes
    )) {
      if (!doesBBoxOverlap(params.sim, shape)) continue;
      for (let row = 0; row < params.sim.type.length; ++row) {
        for (let col = 0; col < params.sim.type[row].length; ++col) {
          if (doesBlockOverlap(params.sim, row, col, shape)) {
            return isActive
              ? WORLD_ACTION.END_THE_GAME
              : WORLD_ACTION.STOP_SHAPE_ACTIONS;
          }
        }
      }
    }
